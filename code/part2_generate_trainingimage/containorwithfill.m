function Curbasic_unit=containorwithfill(Curbasic_unit,s)
    ...
    for k_fill=1:length(suitfillidx)
        ...
        if size(contain_basicimage,1)<willf_row || size(contain_basicimage,2)<willf_col ||...
           ...
           willfill_boxboundings_cur(1)+willf_col-1>size(contain_basicimage,2)
            if willfill_boxboundings_cur(2)+willf_row-1> size(contain_basicimage,1)
                new_row=s;
                willfill_boxboundings_cur(2)=s-willf_row+1;
            else
                new_row=ceil(max(max(size(contain_basicimage,1),willfill_boxboundings_cur(2)+willf_row-1),willf_row));
            end
            if willfill_boxboundings_cur(1)+willf_col-1>size(contain_basicimage,2)
                new_col=s;
                willfill_boxboundings_cur(1)=s-willf_col+1;
            else
...
for k2=1:size(contains_relation_allset,2)
    curcontain=cumsum(contains_relation_allset(1,:)./sum(contains_relation_allset(1,:)));
