...
data_infor_path='E:\2025.228\data\generate_image_training_loveda√\code\new_augmis\data_information\';
% data_infor_path='D:\ljy\generate_image\data_information\';
save_similar_infor='E:\2025.228\data\generate_image_training_loveda√\code\new_augmis\data_information\similar_infor\';

save_micro_path='F:\lly\compare_data_instance\loveda\DRAW_LABEL\';
save_image_path='F:\lly\compare_data_instance\loveda\DRAW_IMAGE\';
...
for k=10:training_set_number
    ...
    % 将混淆类对应的区域数改为1：
    confusedclass_indices=find(ismember(choosepick_classcol,chooseempairs));
    comfusedclass_numarea=choose_classcol_areanum(confusedclass_indices);
    if sum(comfusedclass_numarea)>2
        choose_classcol_areanum(confusedclass_indices)=1;
    end
...
%     end
    ...
    elseif ~isempty(exits_easypaironly) && isempty(exits_easypair)
        ...
        choose_pairidx=randperm(size(remainpair_infor,1),1);
        choose_pairindex=cur_misclass_pair_idx(choose_pairidx,:);
        easypair_index{choose_class,em_toclass}=[easypair_index{choose_class,em_toclass} choose_pairidx];
        for ke=1:length(choose_pairindex)
            curfill_class=choosepair_easy(ke);
    ...
    for k_rand=1:2
        ...
        if size(basic_label,1)~=512
            statscurclass_boundingbox(2)=1024-512+1;
            [basic_image,basic_label]=randcrop_image(Given_infor,sampledpoints_id,statscurclass_boundingbox,curfill_class,curclass_area);
        end
        if  size(basic_label,2)~=512
            statscurclass_boundingbox(1)=1024-512+1;
            [basic_image,basic_label]=randcrop_image(Given_infor,sampledpoints_id,statscurclass_boundingbox,curfill_class,curclass_area);
        end
        strr=strcat('rand_',num2str(k),num2str(k_rand),'.png');
        imwrite(uint8(basic_image),['F:\lly\compare_data_instance\loveda\rand\image\' strr])
        imwrite(uint8(basic_label),['F:\lly\compare_data_instance\loveda\rand\label\' strr])
    end
...
%         figure,imshow(uint8(willfill_image))
        ...
        if sum(label_micro(:))==0%第一种情况：label_micro完全未填充
            ...
            [curbasic_row,curbasic_col]=size(willfill_label);
%             willfill_image_new=willfill_image;
