...
labelpath='D:\A_userfile\lly\data\gid15\label_gid15\';
imageDir=dir([imagepath '*.png']);
labelDir=dir([labelpath '*.png']);
data_infor_path='D:\A_userfile\lly\gid15\update_version_new\data_information\';
% data_infor_path='D:\ljy\generate_image\data_information\';
save_similar_infor='D:\A_userfile\lly\gid15\update_version_new\data_information\similar_infor\';
save_micro_path='D:\A_userfile\lly\gid15\update_version_new\label\';
save_image_path='D:\A_userfile\lly\gid15\update_version_new\image\';
...
% for k_e=1:length(gray_value)
%     for k_c=1:length(gray_value)
%         if k_e~=k_c
%             strr1=strcat('similarity_matrix_',num2str(k_e),'_',num2str(k_c),'.mat');
%             load([save_similar_infor strr1]);
%             field_name = strcat('similarity_matrix_', num2str(k_e), '_', num2str(k_c));
%             simi_struct.(field_name) = similarity_matrix;
%         end
%     end
% end
...
%     strr1=strcat('similarity_matrix_',num2str(choose_class),'_',num2str(em_toclass));
%     simi_matrix=simi_struct.(strr1);
    ...
    for kk=1:length(choose_classcol)
        ...
        if sum(isnan(curclass_toareanum))~=0
    ...
    for kk=1:2
       ...
       stats_condi= cur_chooseclass_area>=chooseclass_meanarea-chooseclass_meanarea*0.01 &...
           cur_chooseclass_area<=s*s*chooseclass_pro+chooseclass_meanarea*0.01;
    ...
    elseif ~isempty(exits_easypaironly) && isempty(exits_easypair)
        ...
        %从中随机抽取一个
        remainpair_idx=setdiff([1:size(cur_misclass_pair_idx,1)],delete_idx);
    ...
    Curbasic_unit=[Curbasic_unit;Curbasic_unit_easy];

    %1.6 combinating basic units into training image
    % 1.6.1 包围关系的填充
    Curbasic_unitupdate=containorwithfill(Curbasic_unit);
    ...
    for k2=1:length(basicunit_fillsort)
       ...
       %% 填充时仅考虑单独一个区域信息
        class_label=Curbasic_unitupdate{willfill_basicid,3};
        ...
        willfill_image=save_image;
        willfill_label=save_label;
        %%
%         figure,imshow(uint8(willfill_image))
        ...
        if sum(label_micro(:))==0%第一种情况：label_micro完全未填充
            ...
            [fill_pixelpoints,theta]=compare_choosepoints(label_micro,Curbasic_unit, willfill_basicid,minbox_diffclass,basic_cen);
...
savepathimage='D:\A_userfile\lly\gid15\update_version_new\imageend\';
savepathlabel='D:\A_userfile\lly\gid15\update_version_new\labelend\';
image_idx=find(cell2mat(cellfun(@(x) isempty(x),withoeasy_classinfor, 'UniformOutput', false)));
imagedirr=dir([save_image_path '*.png']);
for kkk=1155:length(image_idx)
...
imagepath='D:\A_userfile\lly\data\gid5\image_rename\';
labelpath='D:\A_userfile\lly\data\gid15\label_gid15\';
save_image_path='D:\A_userfile\lly\gid15\update_version_new\endimage\';
save_micro_path='D:\A_userfile\lly\gid15\update_version_new\endlabel\';
